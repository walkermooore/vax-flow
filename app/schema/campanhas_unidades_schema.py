...
class PostCampanhasUnidades(BaseModel):

    
    campanha_id: UUID  = Field(..., description="Chave estrangeira referenciando a Campanha")
    unidade_id: UUID  = Field(..., description="Chave estrangeira referenciando a Unidade")

    @model_validator(mode="before")
    @classmethod
    def validators_campanhas_unidades(self, data) -> "PostCampanhasUnidades":
        return data

    @classmethod
    def as_form(
        cls,
        campanha_id: UUID = Form(..., description="Chave estrangeira referenciando a Campanha"),
...
class GetCampanhasUnidades(BaseModel):

    
    campanha_id: UUID = Field(None, description="Chave estrangeira referenciando a Campanha")
...
class PatchCampanhasUnidades(BaseModel):

    
    campanha_id: UUID | None = Field(None, description="Chave estrangeira referenciando a Campanha")
    unidade_id: UUID | None = Field(None, description="Chave estrangeira referenciando a Unidade")

class CampanhasUnidadesInclude(str, Enum):
    campanha_id= "campanha_id"
    unidade_id= "unidade_id"
    id= "id"
    criado_em= "criado_em"
...
class CampanhasUnidadesExpand(str, Enum):
    Nenhum = ""
...
def get_expand_query():
    ...
    members = list(CampanhasUnidadesExpand.__members__.values())
    ...
    if len(members) == 1 and hasattr(CampanhasUnidadesExpand, "Nenhum"):
        return Query(
            None,
            include_in_schema=False,
            description="Este endpoint não possui relacionamentos expansíveis"
    ...
    valid_expansions = [
        ...
        if not (hasattr(CampanhasUnidadesExpand, "Nenhum") and m == CampanhasUnidadesExpand.Nenhum)
    ]
...
class QueryCampanhasUnidades:
    def __init__(
        ...
        # ==========================
        campanha_id: str | None = Query(None, description="Chave estrangeira referenciando a Campanha"),
        ...
        cursor_field: CampanhasUnidadesInclude = Query(
            CampanhasUnidadesInclude.id,
        ...
        # ==========================
        # Controle de retorno
        include: list[CampanhasUnidadesInclude] = Query(
            [], description="Atributos a incluir na resposta"
        ),
        exclude: list[CampanhasUnidadesInclude] = Query(
            [], description="Atributos a excluir da resposta"
        ),
        expand: list[CampanhasUnidadesExpand] | None = get_expand_query(),
        ...
        sort_field: CampanhasUnidadesInclude = Query(
        ...
        self.filters = {}
        fields = [ f.value for f in CampanhasUnidadesInclude]
        for field in fields:
            value = locals()[field]
            if value:
...
QueryCampanhasUnidadesDep = Annotated[QueryCampanhasUnidades, Depends()]