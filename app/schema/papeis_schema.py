...
class PostPapeis(BaseModel):

    
    nome: str  = Field(..., description="Nome identificador único do papeis (ex: 'Admin', 'Gerente')")
    ativo: bool  = Field(True,description="Indica se o papeis está ativo e disponível para atribuição")
    descricao: str | None  = Field(None, description="Descrição detalhada das permissões e responsabilidades do papeis")

    @model_validator(mode="before")
    @classmethod
    def validators_papeis(self, data) -> "PostPapeis":
        if not "nome" in data:
    ...
    def as_form(
        cls,
        nome: str = Form(..., description="Nome identificador único do papeis (ex: 'Admin', 'Gerente')"),
        ativo: bool = Form(..., description="Indica se o papeis está ativo e disponível para atribuição"),
        descricao: str | None = Form(None, description="Descrição detalhada das permissões e responsabilidades do papeis"),
    ):
        return cls(
            nome=nome,
            ativo=ativo,
...
class GetPapeis(BaseModel):

    
    nome: str = Field(None, description="Nome identificador único do papeis (ex: 'Admin', 'Gerente')")
    ativo: bool = Field(None, description="Indica se o papeis está ativo e disponível para atribuição")
    descricao: str | None = Field(None, description="Descrição detalhada das permissões e responsabilidades do papeis")
...
class PatchPapeis(BaseModel):

    
    nome: str | None = Field(None, description="Nome identificador único do papeis (ex: 'Admin', 'Gerente')")
    ativo: bool | None = Field(None, description="Indica se o papeis está ativo e disponível para atribuição")
    descricao: str | None = Field(None, description="Descrição detalhada das permissões e responsabilidades do papeis")

class PapeisInclude(str, Enum):
    nome= "nome"
    ativo= "ativo"
    descricao= "descricao"
    id= "id"
...
def get_expand_query():
    ...
    members = list(PapeisExpand.__members__.values())
    ...
    if len(members) == 1 and hasattr(PapeisExpand, "Nenhum"):
        return Query(
            None,
            include_in_schema=False,
            description="Este endpoint não possui relacionamentos expansíveis"
    ...
    valid_expansions = [
        ...
        if not (hasattr(PapeisExpand, "Nenhum") and m == PapeisExpand.Nenhum)
    ]
...
class QueryPapeis:
    def __init__(
        ...
        # ==========================
        nome: str | None = Query(None, description="Nome identificador único do papeis (ex: 'Admin', 'Gerente')"),
        ativo: str | None = Query(None, description="Indica se o papeis está ativo e disponível para atribuição"),
        descricao: str | None = Query(None, description="Descrição detalhada das permissões e responsabilidades do papeis"),
        ...
        cursor_field: PapeisInclude = Query(
            PapeisInclude.id,
        ...
        # ==========================
        # Controle de retorno
        include: list[PapeisInclude] = Query(
            [], description="Atributos a incluir na resposta"
        ),
        exclude: list[PapeisInclude] = Query(
            [], description="Atributos a excluir da resposta"
        ),
        expand: list[PapeisExpand] | None = get_expand_query(),
        ...
        sort_field: PapeisInclude = Query(
        ...
        self.filters = {}
        fields = [ f.value for f in PapeisInclude]
        for field in fields:
            value = locals()[field]
            if value:
...
QueryPapeisDep = Annotated[QueryPapeis, Depends()]