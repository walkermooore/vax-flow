...
class PostUsuariosFuncionalidades(BaseModel):

    
    usuario_id: UUID  = Field(..., description="Chave estrangeira referenciando o ID do Usuário")
    funcionalidade_id: UUID  = Field(..., description="Chave estrangeira referenciando o ID da Funcionalidades")

    @model_validator(mode="before")
    @classmethod
    def validators_usuarios_funcionalidades(self, data) -> "PostUsuariosFuncionalidades":
        return data

    @classmethod
    def as_form(
        cls,
        usuario_id: UUID = Form(..., description="Chave estrangeira referenciando o ID do Usuário"),
        funcionalidade_id: UUID = Form(..., description="Chave estrangeira referenciando o ID da Funcionalidades"),
    ):
        return cls(
...
class GetUsuariosFuncionalidades(BaseModel):

    
    usuario_id: UUID = Field(None, description="Chave estrangeira referenciando o ID do Usuário")
    funcionalidade_id: UUID = Field(None, description="Chave estrangeira referenciando o ID da Funcionalidades")
...
class PatchUsuariosFuncionalidades(BaseModel):

    
    usuario_id: UUID | None = Field(None, description="Chave estrangeira referenciando o ID do Usuário")
    funcionalidade_id: UUID | None = Field(None, description="Chave estrangeira referenciando o ID da Funcionalidades")

class UsuariosFuncionalidadesInclude(str, Enum):
    usuario_id= "usuario_id"
    funcionalidade_id= "funcionalidade_id"
    id= "id"
    criado_em= "criado_em"
...
class UsuariosFuncionalidadesExpand(str, Enum):
...
def get_expand_query():
    ...
    members = list(UsuariosFuncionalidadesExpand.__members__.values())
    ...
    if len(members) == 1 and hasattr(UsuariosFuncionalidadesExpand, "Nenhum"):
        return Query(
            None,
            include_in_schema=False,
            description="Este endpoint não possui relacionamentos expansíveis"
    ...
    valid_expansions = [
        ...
        if not (hasattr(UsuariosFuncionalidadesExpand, "Nenhum") and m == UsuariosFuncionalidadesExpand.Nenhum)
    ]
...
class QueryUsuariosFuncionalidades:
    def __init__(
        self,
        all_data: bool = Query(
            True,
        ...
        # ==========================
        usuario_id: str | None = Query(None, description="Chave estrangeira referenciando o ID do Usuário"),
        funcionalidade_id: str | None = Query(None, description="Chave estrangeira referenciando o ID da Funcionalidades"),
        ...
        cursor_field: UsuariosFuncionalidadesInclude = Query(
            UsuariosFuncionalidadesInclude.id,
        ...
        # ==========================
        # Controle de retorno
        include: list[UsuariosFuncionalidadesInclude] = Query(
            [], description="Atributos a incluir na resposta"
        ),
        exclude: list[UsuariosFuncionalidadesInclude] = Query(
            [], description="Atributos a excluir da resposta"
        ),
        expand: list[UsuariosFuncionalidadesExpand] | None = get_expand_query(),
        ...
        sort_field: UsuariosFuncionalidadesInclude = Query(
        ...
        self.filters = {}
        fields = [ f.value for f in UsuariosFuncionalidadesInclude]
        for field in fields:
            value = locals()[field]
            if value:
...
QueryUsuariosFuncionalidadesDep = Annotated[QueryUsuariosFuncionalidades, Depends()]